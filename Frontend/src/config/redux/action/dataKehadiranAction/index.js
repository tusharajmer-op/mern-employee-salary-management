...
export const createDataKehadiran = (dataPegawai, dataKehadiran, navigate) => async (dispatch) => {
    try {
        for (let i = 0; i < dataPegawai.length; i++) {
            ...
            if (!isNamaAda) {
                const response = await axios.post("http://localhost:5001/data_kehadiran", {
...
export const updateDataKehadiran = (id, dataKehadiran) => {
    return async (dispatch) => {
        try {
            const response = await axios.put(`http://localhost:5001/data_kehadiran/${id}`, dataKehadiran);
...
export const deleteDataKehadiran = (id) => {
    return async (dispatch) => {
        try {
            const response = await axios.delete(`http://localhost:5001/data_kehadiran/${id}`);
            if (response.status === 200) {
                dispatch({
